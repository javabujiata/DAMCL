...
class GroupCamsSampler(Sampler):
    def __init__(self, dataset, group_n=1, batch_size=None):
        self.dataset = dataset
        self.group_n = group_n
        self.batch_size = batch_size
        self.index_pid = defaultdict(int)
        self.index_cid = defaultdict(int)
        self.pid_cid = defaultdict(list)
        self.pid_index = defaultdict(list)
        label2data_idx = defaultdict(list)
        label2data = defaultdict(list)
        ...
        for index, (_, pid, cid) in enumerate(dataset):
            self.index_pid[index] = pid
            self.index_cid[index] = cid
            self.pid_cid[pid].append(cid)
            self.pid_index[pid].append(index)
    ...
    def __iter__(self):
        ...
        for label, data_idx in self.label2data_idx.items():
            if label != -1:
                data_idx = deepcopy(data_idx)
                random.shuffle(data_idx)
                for i in range(0, len(data_idx), self.group_n):
                    group_idx = []
                    group_data = data_idx[i: i + self.group_n]
                    cams = []
                    for j in group_data:
                        cams.append(self.index_cid[j])
                    index = deepcopy(group_data)

                    cid_idx = defaultdict(list)
                    for j in index:
                        cid_idx[self.index_cid[j]].append(j)


                    max_len = 0
                    for cam, idx in cid_idx.items():
                        if max_len < len(idx):
                            max_len = len(idx)
                    ...
                    for j in range(max_len):
                        ...
                        for cam, idx in cid_idx.items():
                            if j >= len(idx):
                                indexes.append(np.random.choice(idx))
                            else:
                                indexes.append(idx[j])
                        select_indexes.append(indexes)
                    random.shuffle(select_indexes)

                    for select_index in select_indexes:
                        group_idx.extend(select_index)

                    data_idxes.append(group_idx)
