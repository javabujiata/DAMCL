...
from damcl.trainers import Trainer_teacher, Trainer
...
from damcl.utils.data.sampler import RandomMultipleGallerySampler
...
def main_worker(args):
    ...
    sys.stdout = Logger(osp.join(args.logs_dir + "/student_model/" + logging_time[:13], 'log.txt'))
    print("==========\nArgs:{}\n==========".format(args))
    ...
    # Trainer
    model_teacher.train()
    trainer = Trainer(model, model_teacher, log_loss_path=args.log_loss_path, logging_time=logging_time)
    ...
    features, _, _, _, _ = extract_features(model_teacher, cluster_loader)
    ...
    for epoch in range(-1, args.epochs):
        ...
        with torch.no_grad():
            ...
            if epoch == -1:
                features, features_up, features_down, _, cids = extract_features(model_teacher, cluster_loader)
                ...
                features, features_up, features_down, _, _ = extract_features(model, cluster_loader)
        ...
        if (epoch + 1) % args.eval_step == 0 or (epoch == args.epochs - 1):
            ...
            save_checkpoint({
                ...
                'epoch': epoch + 1,
                'best_mAP': best_mAP,
            }, is_best, fpath=osp.join(args.logs_dir + "/student_model/" + logging_time[:13], 'checkpoint.pth.tar'))
    ...
    checkpoint = load_checkpoint(osp.join(args.logs_dir + "/student_model/" + logging_time[:13], 'model_best.pth.tar'))
